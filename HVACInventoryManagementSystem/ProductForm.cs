...
namespace HVACInventoryManagementSystem
{
    public partial class ProductForm : Form
    {
        ...
        public void LoadProduct()
        {
            ...
            cm = new SqlCommand("SELECT * FROM tbProduct WHERE CONCAT(pid,pname,pcategory,pmodelno,pstatus,pwarehouse) LIKE '%" +txtSearch.Text+ "%' ", con);
            con.Open();
            dr = cm.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void dgvProduct_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Edit")
            {
                ...
                productModule.comboCat.Text = dgvProduct.Rows[e.RowIndex].Cells[3].Value.ToString();
                productModule.txtPModelno.Text = dgvProduct.Rows[e.RowIndex].Cells[4].Value.ToString();
                productModule.comboStatus.Text = dgvProduct.Rows[e.RowIndex].Cells[5].Value.ToString();
                productModule.txtPQty.Text = dgvProduct.Rows[e.RowIndex].Cells[6].Value.ToString();
                productModule.txtPPrice.Text = dgvProduct.Rows[e.RowIndex].Cells[7].Value.ToString();
                productModule.txtPWarehouse.Text = dgvProduct.Rows[e.RowIndex].Cells[8].Value.ToString();
